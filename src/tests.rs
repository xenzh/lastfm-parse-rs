...
use lastfm_type::LastfmType;

// ----------------------------------------------------------------

macro_rules! test_fn {
    ($name:ident, $lastfm_type:ident, [$($param_val:expr),*]) => {
        #[test]
        fn $name() {
            ...
            let work = client.get(uri).and_then(|res| {
                res.body().concat2().and_then(move |body: Chunk| {
                    ...
                    let data = $lastfm_type::from_json(&unescaped).map_err(|e| {
...
use super::structs::tag::Info as TagInfo;
